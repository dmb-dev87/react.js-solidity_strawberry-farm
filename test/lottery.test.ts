...
describe("Lottery Contract", () => {
  ...
  beforeEach(async () => {
    ...
    const BuddhatNft = await ethers.getContractFactory("BuddhaNft");
    ...
    buddhaNft = await BuddhatNft.deploy();
    lottery = await Lottery.deploy(buddhaNft.address, ...lotteryParams);
  ...
  describe("Call Test functions to check state", async () => {
    beforeEach(async () => {
      let minter = await buddhaNft.MINTER_ROLE()
      await Promise.all([
        buddhaNft.grantRole(minter, owner.address),
        sbtToken.grantRole(minter, owner.address),
        sbtToken.mint(owner.address, ethers.utils.parseEther("999")),
        buddhaNft.safeMint(alice.address),
        buddhaNft.safeMint(bob.address),
        buddhaNft.safeMint(alice.address),
        buddhaNft.safeMint(bob.address),
        buddhaNft.safeMint(alice.address),
        buddhaNft.safeMint(bob.address),
        sbtToken.approve(lottery.address, ethers.utils.parseEther("25")),
    ...
    it("should call testGetRandomNumber", async () => {
      ...
      expect(await lottery.winningNumber(0))
        .to.eq(3)
      expect(await buddhaNft.tokenOfOwnerByIndex(alice.address, 0))
        .to.eq(0)
    })
    ...
    it("should payout bob", async () => {
      let res = await sbtToken.balanceOf(lottery.address)
      expect(res)
        .to.eq(ethers.utils.parseEther("25"))
      await lottery.testGetWinningNumber()
      await lottery.connect(bob).claimLottoWinnings()
      expect(await sbtToken.balanceOf(lottery.address))
        .to.eq(0)
      expect(await sbtToken.balanceOf(bob.address))
        .to.eq(ethers.utils.parseEther("25"))
    })
